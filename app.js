...
function defaultState() {
  ...
  return {
    ...
    ui: { tab: 'recipes', mode: 'all', mealType: 'all', approvedOnly: false, freezerOnly: false, oscarOnly: false, search: '', expandedId: null, showHave: false },
  };
}
...
function renderRecipes() {
  ...
  container.innerHTML = list.map(r => {
    ...
    const expanded = state.ui.expandedId === r.id;
    ...
    if (expanded) {
      ...
      detail = `
        <div class="recipe-detail">
          ...
          ${meta.timesCooked ? `<div class="meta" style="margin-top:8px;">Cooked ${meta.timesCooked}× · last on ${new Date(meta.lastCooked).toLocaleDateString()}</div>` : ''}
          <div class="btn-row">
            <button class="btn ${inPlan ? 'secondary' : ''}" data-action="toggle-plan" data-id="${r.id}">${inPlan ? '✓ In shopping plan' : '+ Add to shopping plan'}</button>
            <button class="btn secondary" data-action="mark-cooked" data-id="${r.id}">Mark cooked today</button>
          </div>
          <div class="toggle-row">
            <span>👨‍🍳 Oscar can prepare this</span>
            <label class="switch">
              <input type="checkbox" data-action="toggle-oscar" data-id="${r.id}" ${isOscarFriendly(r) ? 'checked' : ''}>
              <span class="track"></span>
            </label>
          </div>
          <div class="btn-row">
            <button class="btn danger" data-action="remove-recipe" data-id="${r.id}" style="flex:none;">🗑️ Remove recipe</button>
          </div>
        </div>`;
    }
  ...
  container.querySelectorAll('.recipe-card').forEach(card => {
    card.addEventListener('click', (e) => {
      ...
      state.ui.expandedId = state.ui.expandedId === id ? null : id;
      save(); renderRecipes();
    });
  });
  container.querySelectorAll('[data-action="toggle-plan"]').forEach(btn => {
    btn.addEventListener('click', () => {
      const id = btn.dataset.id;
      const i = state.planRecipeIds.indexOf(id);
      if (i >= 0) state.planRecipeIds.splice(i, 1); else state.planRecipeIds.push(id);
      save(); renderRecipes();
    });
  });
  container.querySelectorAll('[data-action="mark-cooked"]').forEach(btn => {
    btn.addEventListener('click', () => openRateModal(btn.dataset.id));
  });
  container.querySelectorAll('[data-action="toggle-oscar"]').forEach(cb => {
    cb.addEventListener('change', () => {
      state.oscarOverrides[cb.dataset.id] = cb.checked;
      save(); renderRecipes();
    });
  });
  container.querySelectorAll('[data-action="remove-recipe"]').forEach(btn => {
    btn.addEventListener('click', () => {
      const r = recipeById(btn.dataset.id);
      if (!r) return;
